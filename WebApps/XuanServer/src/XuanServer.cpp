...
XuanServer::XuanServer(int port,
...
void XuanServer::setThreadNum(int numThreads)
{
    httpServer_.setThreadNum(numThreads);
}

void XuanServer::start()
{
    httpServer_.start();
}

void XuanServer::initialize()
{
    // 初始化数据库连接池
...
void XuanServer::initializeSession()
{
    // 创建会话存储
...
void XuanServer::initializeMiddleware()
{
    // 创建中间件
...
void XuanServer::initializeRouter()
{
    // 注册url回调处理器
    // 登录注册入口页面
    ...
    httpServer_.Get("/PVP/start", std::make_shared<PvpStartHandler>(this));
    httpServer_.Post("/PVP/move", std::make_shared<PvpMoveHandler>(this));
    httpServer_.Get("/PVP/poll", std::make_shared<PvpPollMoveHandler>(this));
...
void XuanServer::restartChessGameVsAi(const http::HttpRequest &req, http::HttpResponse *resp)
{
    // 解析请求体
...
// 获取后台数据
void XuanServer::getBackendData(const http::HttpRequest &req, http::HttpResponse *resp)
{
    try 
    {
        // 获取数据
...
void XuanServer::packageResp(const std::string &version,
...
std::unordered_map<int, std::shared_ptr<PVPGame>>& XuanServer::getPvpGames() {
    return pvpGames_;
}

std::unordered_map<int, int>& XuanServer::getMatchedPlayers() {
    return matchedPlayers_;
}

std::queue<int>& XuanServer::getPvpMatchQueue() {
    return pvpMatchQueue_;
}

std::mutex& XuanServer::getPvpGameMutex() {
    return mutexForPvpGames_;
}
